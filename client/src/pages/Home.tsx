    ...
    <div className="min-h-screen bg-[#f3f1ea] text-[#1f2b25]">
      ...
      <main id="top">
        <section className="relative isolate overflow-hidden bg-[#1f2b25] text-[#f3f1ea]">
          <div className="absolute inset-0 bg-[url('/alameda-headshop/images/banner-hero.jpg')] bg-cover bg-center opacity-100" />
        ...
        <section id="colecao" className="bg-[#e8e6dd] px-5 py-20 lg:px-10 lg:py-28"><div className="mx-auto max-w-[1320px]"><div className="flex flex-col justify-between gap-8 border-b border-[#1f2b25]/15 pb-8 sm:flex-row sm:items-end"><div><p className="eyebrow">A seleção Alameda</p><h2 className="mt-3 font-display text-4xl tracking-[-0.05em] sm:text-5xl">Favoritos da casa</h2></div><div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[0.12em]">{["Todos", "Sedas", "Tabacos", "Acessórios", "Combos"].map((item) => <button key={item} onClick={() => setActiveCategory(item)} className={`border-b-2 px-1 pb-2 transition-colors ${activeCategory === item ? "border-[#b1723c] text-[#b1723c]" : "border-transparent text-[#65736a] hover:text-[#1f2b25]"}`}>{item}</button>)}</div></div><div className="mt-10 grid gap-x-4 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">{filteredProducts.map((product) => <article key={product.name} className="group"><div className="relative aspect-[4/4.7] overflow-hidden bg-[#d8d6cd]"><img src={product.image} alt={product.name} className="h-full w-full object-cover mix-blend-multiply transition duration-500 group-hover:scale-105" /><span className="absolute left-3 top-3 bg-[#f3f1ea] px-2 py-1 text-[9px] font-bold uppercase tracking-[0.15em] text-[#65736a]">{product.tag}</span><a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage(product))}`} target="_blank" rel="noreferrer" className="absolute bottom-3 left-3 right-3 bg-[#1f2b25] py-3 text-center text-xs font-bold uppercase tracking-[0.16em] text-[#f3f1ea] transition-colors hover:bg-[#b1723c]">Pedir pelo WhatsApp</a></div><div className="mt-4 flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#9b6b4a]">{product.category}</p><h3 className="mt-1 font-display text-xl tracking-[-0.03em]">{product.name}</h3></div><div className="text-right text-sm font-bold text-[#1f2b25]"><span className={product.oldPrice ? "block text-xs font-normal text-[#65736a] line-through" : "hidden"}>{product.oldPrice}</span>{product.price}</div></div></article>)}</div>{filteredProducts.length === 0 && <p className="py-20 text-center text-[#65736a]">Nenhum produto encontrado para esta busca.</p>}</div></section>

        <section id="instagram" className="bg-[#1f2b25] px-5 py-20 text-[#f3f1ea] lg:px-10 lg:py-24"><div className="mx-auto max-w-[1320px]"><div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow text-[#e6b86b]">Da Alameda para o seu feed</p><h2 className="mt-3 font-display text-4xl tracking-[-0.05em] sm:text-5xl">Siga <em className="font-editorial font-normal text-[#e6b86b]">@alameda.headshop</em></h2></div><a href="https://www.instagram.com/alameda.headshop/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border-b border-[#e6b86b] pb-2 text-xs font-bold uppercase tracking-[0.15em] text-[#f3f1ea] transition-colors hover:text-[#e6b86b]">Ver no Instagram <ArrowRight size={15} /></a></div><div className="instafeed-scroll flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3">{["./images/imgi_41_628151798_18055351037694350_8679542786394916980_n.jpg", "./images/imgi_27_702557241_18067902239694350_4846530733195640862_n.jpg", "./images/imgi_29_689904737_18066488780694350_1464745834191004700_n.jpg", "./images/imgi_35_671119366_18063818762694350_4429845902767154542_n.jpg"].map((image, index) => <a key={image} href="https://www.instagram.com/alameda.headshop/" target="_blank" rel="noreferrer" className="group relative aspect-square min-w-[78%] snap-start overflow-hidden bg-[#304238] sm:min-w-[42%] md:min-w-[24%]"><img src={image} alt={`Publicação da Alameda Headshop ${index + 1}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /></a>)}</div><p className="mt-5 text-xs text-[#aebbb0]">Conteúdo provisório — em breve esta grade poderá ser conectada automaticamente às publicações recentes do Instagram.</p></div></section>

        <section id="alameda" className="mx-auto grid max-w-[1320px] gap-12 px-5 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:items-center lg:px-10 lg:py-28"><div className="relative min-h-[390px] overflow-hidden bg-[#1f2b25]"><div className="absolute inset-8 border border-[#e6b86b]/40" /><div className="absolute -right-8 -top-8 h-56 w-56 rounded-full bg-[#b1723c]/50 blur-3xl" /><div className="relative flex min-h-[390px] items-center justify-center p-12"><img src="./images/imgi_1_Alameda_Headshop_152605.png" alt="Logo Alameda Headshop" className="max-h-full w-full max-w-[500px] object-contain" /></div></div><div className="max-w-xl"><p className="eyebrow">Sobre a Alameda</p><h2 className="mt-4 font-display text-4xl leading-[1.05] tracking-[-0.05em] sm:text-6xl">Curadoria boa é aquela que <em className="font-editorial font-normal text-[#b1723c]">faz sentido.</em></h2><p className="mt-7 text-base leading-7 text-[#65736a]">Cada item que chega por aqui passa por um olhar atento: qualidade, personalidade e a vontade de oferecer uma experiência de compra leve, segura e sem complicação.</p><p className="mt-4 text-base leading-7 text-[#65736a]">A gente acredita em atendimento próximo, novidades constantes e envio discreto — do jeito que tem que ser.</p><a href="mailto:contato.alamedaweed@gmail.com" className="mt-8 inline-flex items-center gap-3 border-b border-[#b1723c] pb-2 text-xs font-bold uppercase tracking-[0.16em] text-[#1f2b25]">Fale com a gente <ArrowRight size={15} /></a></div></section>
      </main>

  ...
  return (
    <div className="min-h-screen bg-[#f3f1ea] text-[#1f2b25]">
      ...
      <main id="top">
        ...
        <section id="instagram" className="bg-[#1f2b25] px-5 py-20 text-[#f3f1ea] lg:px-10 lg:py-24"><div className="mx-auto max-w-[1320px]"><div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow text-[#e6b86b]">Da Alameda para o seu feed</p><h2 className="mt-3 font-display text-4xl tracking-[-0.05em] sm:text-5xl">Siga <em className="font-editorial font-normal text-[#e6b86b]">@alamedaweed</em></h2></div><a href="https://www.instagram.com/alamedaweed/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border-b border-[#e6b86b] pb-2 text-xs font-bold uppercase tracking-[0.15em] text-[#f3f1ea] transition-colors hover:text-[#e6b86b]">Ver no Instagram <ArrowRight size={15} /></a></div><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{["./images/alameda-hero.jpg", "./images/papelitos.jpg", "./images/Tabaco-Santorini-25g.jpg", "./images/acessorios-category.jpg"].map((image, index) => <a key={image} href="https://www.instagram.com/alamedaweed/" target="_blank" rel="noreferrer" className="group relative aspect-square overflow-hidden bg-[#304238]"><img src={image} alt={`Publicação da Alameda Headshop ${index + 1}`} className="h-full w-full object-cover opacity-80 transition duration-500 group-hover:scale-105 group-hover:opacity-100" /><div className="absolute inset-0 bg-[#1f2b25]/20 transition-colors group-hover:bg-transparent" /></a>)}</div><p className="mt-5 text-xs text-[#aebbb0]">Conteúdo provisório — em breve esta grade poderá ser conectada automaticamente às publicações recentes do Instagram.</p></div></section>
